...
class AdminEventVideo extends CI_Controller
{
  public function list()
  {
    ...
    try {
      $rs = array();
      $stmt = \Model::factory('\Models\Video', 'classroom')
        ->select('id')
        ->select('name')
        ->select('description')
        ->select('url')
  ...
  public function save()
  {
    ...
    try {
      ...
      if(count($news) > 0){
				foreach ($news as &$new) {
				  $n = \Model::factory('\Models\Video', 'classroom')->create();
          $n->name = $new->{'name'};
      ...
      if(count($edits) > 0){
          ...
          $e = \Model::factory('\Models\Video', 'classroom')->find_one($edit->{'id'});
          $e->name = $edit->{'name'};
      ...
      if(count($deletes) > 0){
			    ...
			    $d = \Model::factory('\Models\Video', 'classroom')->find_one($delete);
			    $d->delete();
				}
      }
      // commit
