...
class AdminDepartment extends CI_Controller
{
  ...
  public function save()
  {
    ...
    try {
      ...
      if(count($news) > 0){
				foreach ($news as &$new) {
				  $n = \Model::factory('\Models\Department', 'classroom')->create();
					$n->name = $new->{'name'};
					$n->save();
				  $temp = [];
				  $temp['tempId'] = $new->{'id'};
      ...
      if(count($edits) > 0){
          ...
          $e = \Model::factory('\Models\Department', 'classroom')->find_one($edit->{'id'});
					$e->name = $edit->{'name'};
					$e->save();
        }
      }
      ...
      if(count($deletes) > 0){
			    ...
			    $d = \Model::factory('\Models\Department', 'classroom')->find_one($delete);
			    $d->delete();
				}
      }
      // commit
